...
export function AssistanHilo() {
  const { sec, file, id } = useParams();
  ...
  const currentFolder = nav?.find((item) => item.nav_section === id);
...
function Navitem({ section }) {
  return (
    <>
      <h3 className="px-2 py-2 mb-1 text-md text-white  bg-blue-950 font-semibold shadow-md shadow-blue-900">
        {section?.folder}
      </h3>
      <ol>
        {section.sub_section.map((file, index) => (
          <li key={index} className="  hover:bg-zinc-300">
            <NavLink
              className={({ isActive }) => [
                isActive
                  ? "font-bold flex text-md px-2 py-[3px] "
                  : " font-medium flex hover:font-semibold text-md px-2 py-[3px]   cursor-pointer   hover:bg-zinc-500 p-1 border-b border-black ",
              ]}
